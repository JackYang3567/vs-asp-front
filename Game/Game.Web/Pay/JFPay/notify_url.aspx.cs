...
namespace Game.Web.Pay.JFPay
{
	public partial class notify_url : System.Web.UI.Page
	{
		...
		{
			notify_url.ResponseBean responseBean = new notify_url.ResponseBean(base.Request);
			if (!this.GetSign(responseBean).Equals(responseBean.p10_sign))
			{
				Log.Write("签名错误：" + base.Request.Url.ToString());
			}
			else
			{
