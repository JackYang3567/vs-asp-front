...
/*!
 * IE6下Fixed无抖动静止定位
 * 如果您的页面的html元素设定了背景图片请把设置背景图片的css写到body元素上
 * 如果您不需要组件静止定位（也就是随屏滚动）或此段代码影响了您的页面布局可将此段代码删除
 */
_ie6 && (function(bg){
...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	_init: function( config )
	{
		...
		if( config.icon && !isIfr )
		{
			...
			DOM.icon[0].innerHTML = '<img src="' + config.path + 'skins/icons/' + config.icon + '" class="ui_icon_bg" alt=""/>';
		}
		else
...
lhgdialog.tips = function( content, time, icon, callback )
{
	var reIcon = icon ? function(){
	    this.DOM.icon[0].innerHTML = '<img src="' + this.config.path + 'skins/icons/' + icon + '" class="ui_icon_bg" alt=""/>';
